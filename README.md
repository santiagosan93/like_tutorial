...
## Getting started
In this tutorial we will learn how to incorporate likes into your Rails web application. In order to do this we will need the help of the act as votable gem, and some basic understanding of stimulus. It would be good for you to have a refreshner on stimulus by checking the AJAX in Rails lecture. You can see it at 1.5x speed and skip ahead to the second part of the lecture where they speak about stimulus.
...
### Introduction
Ok now that we have everything set up, we can start talking about act_as_votable and how does it work. Act as votable is a Ruby Gem specifically written for Rails/ActiveRecord models. The main goals of this gem are:
...
This votes table generated by act_as_votable is going to be our join table between movies and users. Think that votes can be interpreted as likes.
...
As you can see here we are explicitly telling act_as_votable that our liker is going to be the user, and the model that is going to receive those likes, is the movie.
...
If you added the seeds that come with this tutorial, you can copy paste this lines insede of your ```rails console``` just the way they are. If not, you will have to adapt them to your current needs.

Let's start by makin our first user of our database like a randome movie.
...
As soon as you run this lines, you will see a vote being inserted into our DB. The key values of this vote record is the ```voter_id```, and the ```votable_id```. You will see that the voter_id corresponds to the first user since the user is the voter, and the votable_id will correspond to the id of the movie we're liking.
...
# Stimulus
Now to the fun part. We have everything ready to go, but we need to start adding listeners on our hearts, and make them fire an AJAX request! In order for us to achieve this we will set up stimulus in our app
...
```
Now we have installed stimullus and also we have created our first stimulus controller. Now let's implement a basic stimulus controller skeleton.
...
As much as we would like to see this console log in our page as soon as we refresh, it wont happen since we haven't connected to our stimulus controller from our HTML.
...
Still, this is not enough to have something working since we are specifying in there that the action updateLike will be fired on click, but right now we don't have that action written in our stimulus controller, so that would be the next step.
...
Here we have implemented the create and destroy method to create/delete a like in our databse, and respond with an up to date heart, which is being handled in our success key inside of our Rails.ajax.
